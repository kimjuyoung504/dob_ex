...
classdef ADRC2 < DynSystems
    ...
    methods 
        function obj = ADRC2(name, x0)
            obj = obj@DynSystems(name, x0, true);
        end

        function obj = setParams(obj, b0, oc, order, op_g, r, limit, Td)
            obj.b0 = b0;
            obj.oc = oc;
            obj.order = order;
            obj.r = r;
        ...
        function out = K_pa(obj)
            param = poly(-obj.oc * ones(1, obj.order));
            out = flip(param(2:end));
        end
        
        function u = controller(obj, t, x)
            K = K_pa(obj);
            % u = 1/obj.b0 * (K(1) * obj.r(t) - K * x(1:end-1) - x(end));
            u = 1/obj.b0 * (K(1) * obj.r - K * x(1:end-1) - x(end));
        ...
        % function out = K_pa(obj)
        %     % param = poly(-obj.oc * ones(1, obj.order + 1));
        %     % out = flip(param(3:end));
        %     kd = 2 * obj.oc;
        %     kp = obj.oc^2;
        %     out = [kp kd];
        % end
